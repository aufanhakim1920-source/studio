/* AFTERIMAGE — session replay for web apps
 * ===========================================================================
 * ⭐ THE OBJECT — a long-exposure photograph of the visitor's own cursor.
 *
 * The page holds the shutter open. Wherever the pointer goes, light is added;
 * everywhere else it decays. Move around and you paint a photograph of your own
 * session; stop and it fades. The product records what a user did — the hero
 * records what YOU just did, which is why the object is load-bearing rather
 * than decorative.
 *
 * ── The mechanism: ping-pong frame feedback (Generative Canvas Studio §10.2)
 * Two framebuffers swapped every frame, so the fragment shader can READ the
 * frame it wrote last. WebGL forbids sampling and writing one buffer at once;
 * the swap is the whole trick, and it is what gives a canvas memory.
 *
 *     new frame = warp(previous frame) + residual
 *
 * The note's ratio — 12% new, 88% history — is in the shader verbatim as FRESH
 * and HISTORY. It is written for a per-frame codec smear, which fades in about
 * a tenth of a second; a photographic exposure needs seconds. So the SAME curve
 * is time-stretched:  k = pow(HISTORY, stretch), and the fresh term is
 * renormalised by (1-k)/(1-HISTORY) so the steady state is identical. Same
 * ratio, slower clock.
 *
 * ⚠️ §13.4 — the three ways frame feedback fails SILENTLY. All three look
 * identical from outside (the effect just seems weak), so all three are guarded
 * here explicitly rather than debugged later:
 *   1. an 8-bit history buffer collapses to black   -> float/half-float first,
 *      and if neither exists, a SHORTER exposure whose per-frame decay step is
 *      bigger than one 8-bit quantum (1/255). See PRECISE below.
 *   2. a buffer sized from the wrong pixel measurement dissolves a still frame
 *      -> both targets are sized from cv.width/cv.height and nothing else, and
 *      the only resampling drift is one named constant, DRIFT.
 *   3. a shared depth texture makes the effect never appear at all
 *      -> the framebuffers carry a colour attachment and nothing else.
 *
 * ── Alive before you touch it
 * A recorded session replays on its own: a real pointer path with a hesitation,
 * three rage-clicks and an exit. It paints into the same buffer and it feeds the
 * event rail. When you move, it steps back (lerped, never cut); when you go idle
 * it returns. That is the product demonstrating itself, not an idle animation.
 *
 * ── Other technique carried in
 *   §8.1  pointer influence lerped, never instant
 *   §8.3  tone map with 1-exp(-c), never clamp
 *   §8.5  the canvas carries a CSS fallback of its own image
 *   §10.8 flares store the TIME they started; the shader computes the state
 *   ⚠️ never put a backtick inside a GLSL template literal — it closes the
 *      string silently and the canvas goes blank with no error.
 */
